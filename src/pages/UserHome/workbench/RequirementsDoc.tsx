...
/** 与 Q9 核心空间规划一致：需求书编辑时完整列出，用户仅调整数量 */
...
function fingerprintFromSavedFormData(d: import('../../../types').FormData): Record<string, string> {
  ...
  if (d.requirementsMembers?.length) {
    ...
    ;(['A', 'B', 'C'] as const)
      .filter((r) => r !== d.role)
      .forEach((r) => {
        const spaces = d.otherCoreMemberSpaces?.[r] ?? []
        const note = (d.otherCoreMemberNotes?.[r] ?? '').trim()
        const picked = (d.additionalMembers ?? []).includes(r)
        if (!picked && !spaces.length && !note) return
        list.push({
          id: `core-${r}`,
          name: BASELINE_ROLE_LABELS[r],
          age: '',
          profession: '',
          spaces: spaces.map((name) => ({ name, description: '' })),
          otherActivityNote: note || undefined,
        })
      })
    ;(d.additionalMembers ?? []).forEach((memberId) => {
      if (memberId === 'A' || memberId === 'B' || memberId === 'C') return
      list.push({
        id: memberId,
        ...
        otherActivityNote: (d.additionalMemberNotes?.[memberId] ?? '').trim() || undefined,
      })
    })
...
export function RequirementsDoc({
  ...
  const displayPersonas = useMock
    ...
    : (() => {
        ...
        ;(['A', 'B', 'C'] as const)
          ...
          .forEach((r) => {
            ...
            list.push({
              ...
              height: '',
              stylePersona: null,
              mainActivitiesAndSpaces: spaces,
              otherActivityNote: note || undefined,
              accent: 'slate',
              isStyleTaker: false,
            })
        ...
        ;(d?.additionalMembers ?? []).forEach((memberId) => {
          ...
          const note = (d?.additionalMemberNotes?.[memberId] ?? '').trim()
          list.push({
            name: label,
            age: '',
            profession: '',
  ...
  React.useEffect(() => {
    ...
    if (d.requirementsMembers?.length) {
      ...
      ;(['A', 'B', 'C'] as const)
        .filter((r) => r !== d.role)
        .forEach((r) => {
          const spacesArr = d.otherCoreMemberSpaces?.[r] ?? []
          const note = (d.otherCoreMemberNotes?.[r] ?? '').trim()
          const picked = (d.additionalMembers ?? []).includes(r)
          if (!picked && !spacesArr.length && !note) return
          const roleNm = ROLE_LABELS[r]
          list.push({
            id: `core-${r}`,
            name: roleNm,
            displayName: '',
            age: '',
            profession: '',
            spaces: spacesArr.map((name) => ({ name, description: '' })),
            otherActivityNote: d.otherCoreMemberNotes?.[r] ?? '',
          })
        })
      ;(d.additionalMembers ?? []).forEach((memberId) => {
        if (memberId === 'A' || memberId === 'B' || memberId === 'C') return
        ...
        list.push({
          ...
          otherActivityNote: d.additionalMemberNotes?.[memberId] ?? '',
        })
      })
      setMembersEdit(list)
    }
  ...
  return (
    <div className={snapshotEmbedded ? 'space-y-6 pb-4' : 'space-y-8 pb-24'}>
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between gap-4">
          <SectionTitle title="成员画像（Q5 核心成员）" />
          {hasMemberData && !(isEditing && updateData && !useMock) ? (
            <div className="hidden md:flex items-center gap-2 text-xs text-gray-500">
              <Users size={14} className="text-gray-400" />
              <span className="font-semibold text-gray-400">Q5</span>
        ...
        {isEditing && updateData && !useMock ? (
          ...
          <div className="rounded-2xl border border-dashed border-gray-200 bg-[#FFFDF3] p-8 text-center">
            ...
            <div className="mt-1 text-sm text-gray-600">
              请先完成深度测评中的「核心成员」（Q5）与「家庭成员」（Q6），或进入编辑模式直接添加成员。
            </div>
          </div>
        )}
      </section>

    ...
    <div className="bg-white rounded-[24px] shadow-card hover:shadow-xl transition-all duration-300 p-6 mb-24 border border-[#E5E7EB]">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-[30px] font-black text-[#0A0A0A]">高端设计结算明细</h2>
          </div>
        </div>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b-2 border-[#4887FF]/20 bg-blue-50">
                ...
                <th className="text-right py-4 px-4 text-[12px] font-bold text-[#0A0A0A] whitespace-nowrap bg-blue-100/50">结算数量</th>
                <th className="text-right py-4 px-4 text-[12px] font-bold text-[#0A0A0A] whitespace-nowrap bg-blue-100/50">结算金额</th>
            ...
            <tbody>
              {items.map((item, index) => (
                ...
                >
                  ...
                  <td className="py-4 px-4 text-[16px] font-bold text-[#4887FF] text-right bg-blue-50/50">{item.settlementQuantity}</td>
                  <td className="py-4 px-4 text-[16px] font-bold text-[#4887FF] text-right bg-blue-50/50">{item.settlementAmount.toLocaleString()}</td>
            ...
            <tfoot>
              <tr className="bg-blue-50 border-t-2 border-[#4887FF]/30">
                <td colSpan={9} className="py-4 px-4 text-[16px] font-bold text-[#0A0A0A] text-right rounded-bl-xl">设计费用结算合计：</td>
                <td colSpan={2} className="py-4 px-4 text-[30px] font-black text-[#4887FF] text-right rounded-br-xl">¥{total.toLocaleString()}</td>
              </tr>
            </tfoot>
          </table>
        </div>
